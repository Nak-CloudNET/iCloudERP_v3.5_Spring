...
class Pos extends MY_Controller
{
	...
	function close_register_popup($register_id){
		...
		$q = $this->db->get();
		if($q->num_rows()>0){
			$this->close_register($q->row()->user_id,$q->row()->date, $q->row()->id, $q->row()->total_cash);
		}
		return false;
	}
