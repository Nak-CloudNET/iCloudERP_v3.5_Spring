	...
	*/	
	...
	public function suggestionsStock()
	{
		...
		if($rows){
			...
			foreach ($rows as $row) {
				...
				$pr[] = array('id' => ($c + $r), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")",'code' => $row->code, 'name' => $row->name, 'qoh' => $row->qoh, 'quantity' => $row->quantity, 'unit_name' => $row->unit_name, 'cost' => $row->cost, 'unit' => $row->unit, 'option_unit' => $option_unit, 'project_qty' => $project_qty);
            }
	...
	public function add_using_stock()
	{
        ...
        if ($this->form_validation->run() == true){
			...
			for ($r = 0; $r < $i; $r++) {
				...
				if($plan){
					$proItem = $this->products_model->getProjectPlanItem($plan, $product_id);
					if($qty_balance > $proItem->quantity_balance){
						$this->session->set_flashdata('error', $this->lang->line("qty_big_pro") );
						redirect($_SERVER["HTTP_REFERER"]);
					}
				}
	...
	public function get_using_stock()
    {
		...
		$action_link = '<div class="btn-group text-left"><button type="button" class="btn btn-default btn-xs btn-primary dropdown-toggle" data-toggle="dropdown">'.lang("actions").'<span class="caret"></span></button>
			<ul class="dropdown-menu pull-right" role="menu">					        									
				<li><a href="'.site_url('products/edit_using_stock_by_id/$1/$2').'" ><i class="fa fa-edit"></i>'.lang('edit_using_stock').'</a></li>	
				<li><a href="'.site_url('products/return_enter_using_stock_by_id/$1/$2').'" ><i class="fa fa-reply"></i>'.lang('return_using_stock').'</a></li>	
