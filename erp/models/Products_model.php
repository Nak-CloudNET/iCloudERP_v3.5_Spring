...
class Products_model extends CI_Model
{
	...
	public function getUsingStockProducts($term, $warehouse_id, $plan = NULL, $limit = 5)
    {
		...
		$this->db->where("warehouses_products.warehouse_id", $warehouse_id);
        $this->db->limit($limit);
		$this->db->select('products.*, warehouses_products.quantity as qoh, units.name as unit_name, COALESCE(erp_project_plan_items.quantity_balance, 0) as project_qty');
		$this->db->join('warehouses_products', 'warehouses_products.product_id = products.id', 'left');
	...
	{
        $this->syncUsingStock($data['reference_no']);
	...
	private function syncUsingStock($ref){
		...
		foreach($using_item as $item){
			...
			$this->db->delete("inventory_valuation_details",array('field_id'=>$item->id));
