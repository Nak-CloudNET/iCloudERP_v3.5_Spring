...
class Sales_model extends CI_Model
{
    ...
    {
        $this->db->select('products.id, start_date, end_date, erp_products.code, erp_products.name, erp_products.type, cost, warehouses_products.product_id, warehouses_products.quantity AS qoh, warehouses_products.quantity, categories.type AS cate_type, price, tax_rate, tax_method, erp_products.image, promotion, promo_price, product_details, details, subcategory_id, cf1, COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) FROM erp_serial as sp WHERE sp.product_id='.$this->db->dbprefix('products').'.id), "") as sep')
	...
	public function getProductNumber($term, $warehouse_id, $standard, $combo, $digital, $service, $category, $limit = 20)
    {
		if(preg_match('/\s/', $term))
		{
			$name = explode(" ", $term);
			$first = $name[0];
			$this->db->select('products.id, products.code, products.name, products.type, cost,warehouses_products.quantity, price, tax_rate, tax_method, cf1, product_details,subcategory_id, categories.type AS cate_type, details,COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) 
			...
			$this->db->select('products.id, products.code, products.name, products.type, cost, warehouses_products.quantity, price, tax_rate, tax_method, product_details, details, COALESCE((SELECT GROUP_CONCAT(sp.`serial_number`) FROM erp_serial as sp WHERE sp.product_id='.$this->db->dbprefix('products').'.id ), "") as sep, cf1')
			    ->join('categories', 'categories.id=products.category_id', 'left')
            ->join('warehouses_products', 'warehouses_products.product_id=products.id', 'left')
            ->group_by('products.id');
			if($this->Owner || $this->admin){
  ...
  //           ->group_by('deliveries.id')
	...
	public function getDeliveryItemsByItemIds($delivery_id = NULL)
	{
		...
		$response = $this->db
				 ...
				 ->join('erp_delivery_items','erp_delivery_items.sale_id = erp_sale_order_items.sale_order_id AND erp_delivery_items.product_id = erp_sale_order_items.product_id')
                 ->from('erp_sale_order_items')
	...
	public function getAllInvoiceReItems($sale_id)
    {
        $this->db->select("sale_items.*,
        				...
        				")
            ->join('return_items', 'sale_items.sale_id = return_items.sale_id', 'left')
            ->join('products', 'products.id=sale_items.product_id', 'left')
            ->join('product_variants', 'product_variants.id=sale_items.option_id', 'left')
            ->join('tax_rates', 'tax_rates.id=sale_items.tax_rate_id', 'left')
			->join('categories', 'categories.id = products.category_id', 'left')
