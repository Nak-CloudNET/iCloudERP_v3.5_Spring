            ...
            <div class="-table-responsive">
                <table class="table table-bordered table-hover table-striped" style="width: 100%;">
					...
					if(isset($return_sale)){
						if ($return_sale && $return_sale->surcharge != 0) {
							echo '<tr><td colspan="5"></td><td colspan="3" style="text-align:right;">' . lang("surcharge") . '</td><td style="text-align:right;">' . $this->erp->formatMoney($return_sale->surcharge) . '</td></tr>';
						}
					}
					
                    ?>
                    <?php if ($inv->order_discount != 0) {
                        // echo '<tr><td colspan="3" style="text-align:right;">' . lang("order_discount") . '</td><td style="text-align:right;"><span class="pull-left">'.($discount_percentage?"(" . $discount_percentage . ")" : '').'</span>' . $this->erp->formatMoney($inv->order_discount) . '</td></tr>';

                        echo '<tr><td colspan="3" style="text-align:right;">' . lang("order_discount") . '</td><td style="text-align:right;">' . $this->erp->formatMoney($inv->order_discount) . '</td></tr>';
                    }
                    ?>
					<?php if ($inv->shipping != 0) {
                        echo '<tr><td colspan="3" style="text-align:right;">' . lang("shipping") . '</td><td style="text-align:right;">' . $this->erp->formatMoney($inv->shipping) . '</td></tr>';
                    }
                    ?>
                    <?php if ($Settings->tax2 && $inv->order_tax != 0) {
                        echo '<tr><td colspan="3" style="text-align:right;">' . $inv->tax_name .'</td><td style="text-align:right;">' . $this->erp->formatMoney($inv->order_tax) . '</td></tr>';
                    }
                    ?>
