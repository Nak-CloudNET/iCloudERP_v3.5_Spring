...
$(document).on('change', '.qty_use', function () {
	...
	var qty_use		= $(this).val();
	if(qty_project > 0){
		var bal_project = qty_project - (qty_use * (qty_unit?qty_unit:1));
		row.find('.bal_pro').text(bal_project);
	}
});
...
function loadItems() {
    if (localStorage.getItem('poitems')) {
        ...
        $.each(poitems, function () {
			...
			var opt = $("<select id=\"unit\" name=\"unit\[\]\" style=\"padding-top: 2px !important;\" class=\"form-control select\" />");
			...
			tr_html += '<td><input type="hidden" value="" class="form-control" name="qty_plan[]" style="text-align:center !important;"/><input type="hidden" value="" class="form-control" name="qty_used[]" style="text-align:center !important;"/><input type="text" value="'+ item_qty_by_unit +'" class="form-control qty_use" name="qty_use[]" style="text-align:center !important;"/><input type="hidden" value="'+ item_proj +'" class="qty_project" name="qty_project[]" /><input type="hidden" value="'+qoh+'" name="qoh[]" /></td>';
