...
class Sales extends MY_Controller
{
	...
	function getSales($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
			...
			$this->datatables
				->select("sales.id, sales.date as date,erp_quotes.reference_no as q_no, sale_order.reference_no as so_no, sales.reference_no as sale_no, sales.biller, group_areas.areas_group, sales.customer, users.username AS saleman, sales.sale_status, sales.grand_total, (SELECT SUM(IF(erp_payments.paid_by = 'deposit', erp_payments.amount, 0)) FROM erp_payments WHERE erp_payments.sale_id = erp_sales.id  ) as deposit, COALESCE((erp_sales.paid - (SELECT SUM(IF(erp_payments.paid_by = 'deposit', erp_payments.amount, 0)) FROM erp_payments WHERE erp_payments.sale_id = erp_sales.id)),0), (ROUND(erp_sales.grand_total, 2)-erp_sales.paid) as balance, sales.payment_status")
				->from('sales')
				->join('users', 'users.id = sales.saleman_by', 'left')
				->join('sale_order', 'sale_order.id = sales.so_id', 'left')
	...
	function getSales_DuePayment($warehouse_id = NULL, $dt = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("id, date, due_date, reference_no, biller, customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status")
                ->from('sales')
				->where('payment_status !=', 'paid')
				->where('sale_status !=', 'returned');
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('return_sales') . ".id as id," . $this->db->dbprefix('return_sales') . ".date as date, " . $this->db->dbprefix('return_sales') . ".reference_no as ref, (
						...
						END
					) AS sale_ref," . $this->db->dbprefix('return_sales') . ".biller, " . $this->db->dbprefix('return_sales') . ".customer, " . $this->db->dbprefix('return_sales') . ".surcharge, " . $this->db->dbprefix('return_sales') . ".grand_total, " . $this->db->dbprefix('return_sales') . ".paid, (COALESCE(" . $this->db->dbprefix('return_sales') . ".grand_total, 0) - COALESCE(" . $this->db->dbprefix('return_sales') . ".paid, 0)) AS balance")
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
					...
					$this->excel->getActiveSheet()->SetCellValue('F1', lang('group_area'));
                    $this->excel->getActiveSheet()->SetCellValue('G1', lang('customer'));
                    $this->excel->getActiveSheet()->SetCellValue('H1', lang('saleman'));
                    $this->excel->getActiveSheet()->SetCellValue('I1', lang('sale_status'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('J' . $row, $sale->grand_total);
                        $this->excel->getActiveSheet()->SetCellValue('K' . $row, $sale->deposit);
                        $this->excel->getActiveSheet()->SetCellValue('L' . $row, $sale->paid);
                        $this->excel->getActiveSheet()->SetCellValue('M' . $row, $balance);
                        $this->excel->getActiveSheet()->SetCellValue('N' . $row, $sale->payment_status);
                        $new_row = $row+1;
                            $this->excel->getActiveSheet()->SetCellValue('J' . $new_row, $sum_grand);
                            $this->excel->getActiveSheet()->SetCellValue('K' . $new_row, $sum_deposit);
                            $this->excel->getActiveSheet()->SetCellValue('L' . $new_row, $sum_paid);
                            $this->excel->getActiveSheet()->SetCellValue('M' . $new_row, $sum_banlance);
                        $row++;
                    }
                }else{
                    // echo "user";exit();
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $sale->reference_no." ");
                        $this->excel->getActiveSheet()->SetCellValue('E' . $row, $sale->biller);
						$this->excel->getActiveSheet()->SetCellValue('F' . $row, $sale->group_area);
                        $this->excel->getActiveSheet()->SetCellValue('G' . $row, $sale->customer);
                        $this->excel->getActiveSheet()->SetCellValue('H' . $row, $sale->saleman);
                        $this->excel->getActiveSheet()->SetCellValue('I' . $row, $sale->sale_status);
                        $this->excel->getActiveSheet()->SetCellValue('J' . $row, $sale->grand_total);
	...
	function add_payment_loans($data = NULL,$id = NULL,$paid_amount = NULL,$principle = NULL,$interest = NULL,$sale_id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			for($i=0; $i<sizeof($arr_id)-1; $i++){
				...
				$loans = array(
					...
					'period' => $arr_id[$i],
					'reference_no' => $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->site->getReference('sp'),
					'paid_amount' => $arr_paid[$i],
					'paid_by' => $this->input->post('paid_by'),
					'note' => $this->input->post('note'),
					'created_by' => $this->session->userdata('user_id')
				);
				$curr_paid += $arr_principle[$i];
				if($this->sales_model->addPaymentLoan($loans,$sale_id)) {
					$sale_loan = $this->sales_model->getSaleId($arr_id[$i]);
					$help = true;
				}
			}
	...
	function getCurrentStockQtyByDate(){
		...
		$curStock = $this->sales_model->getCurrentStockQuantityByDate($expiry,$product_code,$warehouse_id);
